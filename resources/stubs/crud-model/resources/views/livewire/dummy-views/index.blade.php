...
<x-layouts.page>
    <div class="d-lg-flex justify-content-between">
        ...
        <div class="d-flex gap-2 mb-3">
            <x-bs::button type="button" icon="plus" :title="__('Create')"
                wire:click="$emit('showModal', 'dummy-route-name.save')"/>
        ...
        @forelse($dummyModelVariables as $dummyModelVariable)
            <div class="d-grid d-lg-flex align-items-center list-group-item list-group-item-action gap-2">
                <div class="col-lg">
                    ...
                    <small class="d-block text-muted">{{ $dummyModelVariable->created_at }}</small>
                </div>
                <div class="d-flex col-lg-auto gap-2">
                    <x-bs::button type="button" icon="eye" :title="__('Read')" color="link" class="p-0"
                        wire:click="$emit('showModal', 'dummy-route-name.read', {{ $dummyModelVariable->id }})"/>

                    <x-bs::button type="button" icon="pencil-alt" :title="__('Update')" color="link" class="p-0"
                        wire:click="$emit('showModal', 'dummy-route-name.save', {{ $dummyModelVariable->id }})"/>
    ...
    <x-bs::pagination :links="$dummyModelVariables" justify="end"/>
</x-layouts.page>
